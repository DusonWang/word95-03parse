 ...
 * @throws Exception 
  ...
  public byte[] expandpw (String password)
  {
      ...
      char []temp=utf8ToUnicode(password).toCharArray();
      for(int j=0;j<16;j++){
    	  if(j<temp.length) passwd[j]=temp[j];
    	  else passwd[j]=0;
      }
      while (passwd[i]!=0)
        {
    	  pwarray[(2 * i)] =  (byte) (passwd[i] & 0xff);
    	  pwarray[(2 * i)+1 ] =  (byte) ((passwd[i] <<8) & 0xff);
    	  i++;
        }
  ...
  public HWPFDocument(DirectoryNode directory,String password) throws Exception
  {
    ...
    {        
		  for (i = 0; i < 0x10; i++){
			  if(k+i>=end-1) { test[i]=0; continue;}
